    ...
    def train(self, trial, train_loader, val_loader, batch_size, n_epochs, n_features=1):
        ...
        for epoch in range(1, n_epochs + 1):
           ...
           #wandb.log({"Train_loss": {"training loss": training_loss}})
            ...
            #wandb.run.summary["Final Validation Loss"] = validation_loss
...
def readData():
    ...
    # X (features)
    lstm_df_scaled['x_gps'] = x1_scaler.fit_transform(df_lstm['x_gps'].to_numpy().reshape(-1, 1))
    lstm_df_scaled['y_gps'] = x2_scaler.fit_transform(df_lstm['y_gps'].to_numpy().reshape(-1, 1))
    lstm_df_scaled['IMU_x'] = x3_scaler.fit_transform(df_lstm['IMU_x'].to_numpy().reshape(-1, 1))
    lstm_df_scaled['IMU_y'] = x4_scaler.fit_transform(df_lstm['IMU_y'].to_numpy().reshape(-1, 1))
    lstm_df_scaled['gyro'] = x5_scaler.fit_transform(df_lstm['gyro'].to_numpy().reshape(-1, 1))

    return (lstm_df_scaled, df_lstm, x1_scaler, x2_scaler, x3_scaler, x4_scaler, x5_scaler, y1_scaler, y2_scaler)
...
def plotResults(df_lstm, df_result):
    ...
    #gt_path = pd.DataFrame(df_lstm, columns = ['x_gt','y_gt']).drop(list(range(drop_range[0], drop_range[1])), axis=0)
    #gps_path = pd.DataFrame(df_lstm, columns = ['x_gps','y_gps']).drop(list(range(drop_range[0], drop_range[1])), axis=0)
...
def trainModelStartToFinish(trial, lr, epochs, num_layers, hidden_size, append_dataset_list=[], N=0, display=True):
    ...
    #big_tuple = splitData(lstm_df_scaled, df_lstm, train_size=13530, val_size=5412, test_size=5412)
    big_tuple = splitData(lstm_df_scaled, df_lstm, train_size=1624, val_size=541, test_size=541)
    ...
    #model = LSTM_Model(input_size=n_features, output_size=2, hidden_dim=36, n_layers=1)
    model = LSTM_Model(input_size=n_features, output_size=2, hidden_dim=hidden_size, n_layers=num_layers)
...
def objective(trial):
    ...
    model, result_metrics = trainModelStartToFinish(trial, lr=lr, epochs=n_epochs, num_layers=num_layers,
                                                    hidden_size=hidden_size, append_dataset_list=[], N=0, display=False)
...
# value: 8.822477340698242 and parameters: {'lr': 0.00018868435800919247, 'n_epochs': 250, 'hidden_size': 256, 'weight_decay': 2.0676999666529862e-07}
...
def run_model(trial=0,lr=0.00008, epochs=10, num_layers=1, hidden_size=512, append_dataset_list=[], N=0, display=True):
